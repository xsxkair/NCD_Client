...
/*	@Query(value="SELECT t.result,COUNT(t.id) FROM TestData t where DATE(t.testtime)=CURRENT_DATE and t.deviceid in (:devicelist) GROUP BY (t.result)")
	...
	//视图月，分组项目
	@Query("select t.card.item, DATE_FORMAT(t.testtime,'%Y-%m'), count(t.id) from TestData t where t.deviceid in (:devices) group by t.card.item, DATE_FORMAT(t.testtime,'%Y年%m月')")
	public List<Object[]> queryReportSummyByMonthItem(@Param("devices")List<Integer>devices);
	
	//视图日，分组项目
	@Query("select t.card.item, DATE_FORMAT(t.testtime,'%Y-%m-%d'), count(t.id) from TestData t where t.deviceid in (:devices) group by t.card.item, DATE_FORMAT(t.testtime,'%Y年%m月%d日')")
