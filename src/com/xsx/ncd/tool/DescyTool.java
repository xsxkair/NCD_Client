...
public class DescyTool {
	...
	public static String DisDes(String string){
		...
		/*∂¡»°key*/
		int[] key = new int[54];
		int[][] word = new int[94][94];
		
		StringBuffer result = new StringBuffer();
		
		try {
			for (int i = 0; i < myKey.length(); i++) {
				key[i] = myKey.charAt(i);
			}
			...
			for (int i = 0; i < 94; i++) 
			{
				for (int j = 0; j < 94; j++) 
				{
					word[i][j] = myWord[i].charAt(j);
				}
			}

			for(int i=0; i<string.length(); i++)
			{
				int index = (i+string.length())%key.length;
	    		int temp1 = key[index]-33;
	    		int temp2 = string.charAt(i);
				
				for (int j = 0; j < 94; j++) 
				{
					if(word[temp1][j] == temp2)
					{
	...
	public static String Des(String string){
		...
		try {
	    	...
	    	for (int i=0; i<string.length(); i++) {
	    		int index = (i+string.length())%key.length;
	    		int temp1 = key[index]-33;
	    		int temp2 = string.charAt(i)-33;
				result.append(new Character((char) word[temp1][temp2]));
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
